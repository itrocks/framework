	...
	public function testReverse()
	{
		...
		$this->assume(
			...
			Joins::newInstance(Salesman::class)
				->addMultiple(['SAF\Order->salesmen.number', 'name'])
				->getJoins(),
			[
				'SAF\Order->salesmen-link' => Join::newInstance(
					Join::LEFT, 't0', 'id', 't1', 'orders_salesmen', 'id_salesman'
				),
				'SAF\Order->salesmen' => Join::newInstance(
					Join::LEFT, 't1', 'id_order', 't2', 'orders', 'id',
					Join::SIMPLE, Order::class
				),
				'SAF\Order->salesmen.number' => null,
				'name' => null
			]
		);
	}
