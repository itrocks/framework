...
abstract class Aop
{
	 ...
	 *        array("class_name"|object, "methodName")
	...
	private static function addMethodCall($joinpoint, $advice, $code, $result_as_arg)
	{
		...
		if ($trait->isTrait()) {
			...
			$arguments = (new Reflection_Method($joinpoint[0], $joinpoint[1]))->getArguments();
			$arguments_names = array_keys($arguments);
			$method_arguments = join(", ", $arguments);

			$advice_string = (is_object($advice[0]))
				? "array(" . __CLASS__ . "::\$joinpoints[$count][0], '$advice[1]')"
				: "array('$advice[0]', '$advice[1]')";
			...
			$code = str_replace(
				...
				array($advice_string, $joinpoint[1], $count, $advice_arguments, $process_arguments, "'" . (is_object($joinpoint[0]) ? get_class($joinpoint[0]) : $joinpoint[0]) . "'"),
				$code
			);
		...
		if (!isset(Aop::$properties[$class_name])) {
			...
			if (!method_exists($class_name, '__aop')) {
				...
				if (method_exists($class_name, '__construct')) {
					...
					$arguments = $__construct->getArguments();
					$method_arguments = join(", ", $arguments);
			...
			runkit_method_add($class_name, '__get', '$property', '
				...
				if (isset(' . $aop_properties . '[$property]["read"])) {
					$joinpoint = new SAF\Framework\Property_Read_Joinpoint("' . (is_object($joinpoint[0]) ? get_class($joinpoint[0]) : $joinpoint[0]) . '", $this, $property);
			...
			runkit_method_add($class_name, '__set', '$property, $value', '
				...
				if (isset(' . $aop_properties . '[$property]["write"])) {
					$joinpoint = new SAF\Framework\Property_Write_Joinpoint("' . (is_object($joinpoint[0]) ? get_class($joinpoint[0]) : $joinpoint[0]) . '", $this, $property);
