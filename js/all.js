// problem : path is relative to the caller, and can change !
// use the global path to set it before calling all.js (sorry, i've no better idea for the moment)

$.getScript(path + 'saf/framework/js/saf.constants.js');
$.getScript(path + 'vendor/components/jqueryui/jquery-ui.js');
$.getScript(path + 'saf/framework/js/jquery.xtarget.js');

$.getScript(path + 'vendor/alvaro-prieto/colResizable/colResizable-1.6.min.js');
$.getScript(path + 'vendor/malsup/form/jquery.form.js');

$.getScript(path + 'saf/framework/js/jquery.aform.js');
$.getScript(path + 'saf/framework/js/jquery.autofocus.js');
$.getScript(path + 'saf/framework/js/jquery.autoheight.js');
$.getScript(path + 'saf/framework/js/jquery.autowidth.js');
$.getScript(path + 'saf/framework/js/jquery.build.js');
$.getScript(path + 'saf/framework/js/jquery.classvar.js');
$.getScript(path + 'saf/framework/js/jquery.modifiable.js');
$.getScript(path + 'saf/framework/js/jquery.mouse.js');
$.getScript(path + 'saf/framework/js/jquery.presstab.js');
$.getScript(path + 'saf/framework/js/jquery.sortcontent.js');
$.getScript(path + 'saf/framework/js/jquery.verticalscrollbar.js');
$.getScript(path + 'saf/framework/js/jquery-ui.tabber.js');

$.getScript(path + 'saf/framework/js/saf.app.js');
$.getScript(path + 'saf/framework/js/saf.fullheight.js');
$.getScript(path + 'saf/framework/js/saf.functions.js');
$.getScript(path + 'saf/framework/js/saf.string.js');

$.getScript(path + 'saf/framework/debug/debug.js');
$.getScript(path + 'saf/framework/import_export/import.js');
$.getScript(path + 'saf/framework/printer/model/model.js');
$.getScript(path + 'saf/framework/property/select.js');
$.getScript(path + 'saf/framework/rad/output.js');
$.getScript(path + 'saf/framework/rad/rad.js');
$.getScript(path + 'saf/framework/settings/custom.js');
$.getScript(path + 'saf/framework/view/logger/html_logger.js');
$.getScript(path + 'saf/framework/widget/edit/edit.js');
$.getScript(path + 'saf/framework/widget/data_list/dataList.js');
$.getScript(path + 'saf/framework/widget/trashcan/trashcan.js');

$.getScript(path + 'saf/framework/js/default.application.js');
