		...
		if (parent_top < window.scrollbar.top()) {
			...
			if ($fixed.length) {
				...
				$fixed2.css({
					'margin-bottom':  0,
					'margin-top':     0,
					'padding-bottom': parseInt($next.css('margin-bottom') + parseInt($next.css('padding-bottom'))) + 'px',
					'padding-top':    parseInt($next.css('margin-top')    + parseInt($next.css('padding-top')))    + 'px'
				});
