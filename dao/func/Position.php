...
class Position implements Where
{
	...
	public function toHuman(Summary_Builder $builder, $property_path, $prefix = '')
	{
		...
		. '('
		. ($this->needle instanceof Where ? $this->needle->toHuman($builder, $property_path, $prefix) : Value::escape($this->needle)) . ','
		. ($this->haystack instanceof Where ? $this->haystack->toHuman($builder, $property_path, $prefix) : Value::escape($this->haystack)) . ','
		. ($this->offset)
		. ')';
	}
	...
	public function toSql(Builder\Where $builder, $property_path, $prefix = '')
	{
		...
		. '('
		. ($this->needle instanceof Where ? $this->needle->toSql($builder, $property_path, $prefix) : Value::escape($this->needle)) . ','
		. ($this->haystack instanceof Where ? $this->haystack->toSql($builder, $property_path, $prefix) : Value::escape($this->haystack)) . ','
		. ($this->offset)
		. ')';
	}
